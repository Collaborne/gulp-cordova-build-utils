...
module.exports = function injectConfig({ appId, versionNumber, source = DEFAULT_SOURCE} = options) {
	...
	const androidVersionCode = Number(versionTokens[0])*100000 + Number(versionTokens[1])*1000 + Number(versionTokens[2])*10;
	...
	return lazypipe()
		...
		.pipe(() => replace('%%GULP.CORDOVA_VERSION_ANDROID_VERSION_CODE%%', String(androidVersionCode)))
